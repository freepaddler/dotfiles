...
Both server and client certificates are signed by intermediate CA.
...
openssl req \
    ...
    -x509 -days 365 -CA "$DEST"/ca.crt -CAkey "$DEST"/ca.key \
...
openssl req \
    ...
    -x509 -days 180 -CA "$DEST"/ca-int.crt -CAkey "$DEST"/ca-int.key \
